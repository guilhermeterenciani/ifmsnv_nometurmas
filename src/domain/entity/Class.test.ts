...
it('shoud create a class',()=>{
    ...
    const class_ = new Class(null,className,nickname);
...
it('shoud not create a class without className',()=>{	
    const className = '';
    const nickname = '1021-B';
    expect(()=>new Class(null,className,nickname)).toThrowError('Nome da turma não pode ser vazio');
})
it('shoud not create a class without nickname',()=>{
    const className = '20231098105BIN15KNV-COINF-Frameworks 1A';
    const nickname = '';
    expect(()=>new Class(null,className, nickname)).toThrowError('Apelido da turma não pode ser vazio');
})
